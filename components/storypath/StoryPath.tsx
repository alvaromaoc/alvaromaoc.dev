                ...
                {items.map((item, index) => (
                    <li key={index} onClick={(e) => storyPointClick(e, index)}
                        ...
                        <div
                            ...
                            <span
                                className={`h-[1px] flex-1 md:-ml-5 md:mt-0 -mt-5 w-1/2 border ${index === 0 ? 'border-transparent' : 'border-body'}`}/>
                            <span
                                className={`absolute left-[calc(50%-0.5rem)] top-[calc(50%-0.5rem)] w-[1rem] h-[1rem] rounded-full border border-body ${activeStory === index ? 'bg-primary-accent' : 'bg-primary'}`}/>
                            <span
                                className={`h-[1px] flex-1 md:-mr-5 md:mb-0 -mb-5 w-1/2 border ${index === items.length - 1 ? 'border-transparent' : 'border-body'}`}/>
                        </div>
                    </li>
                ))}
            </ul>
